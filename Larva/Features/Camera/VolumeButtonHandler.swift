...
// From https://github.com/CipherBitCorp/VolumeButtonHandler
...
public class VolumeButtonHandler: NSObject {
    ...
    public func stopHandler() {
        ...
        isStarted = false
        volumeView?.isHidden = false
        session?.removeObserver(self, forKeyPath: VolumeButtonHandler.sessionVolumeKeyPath)
    ...
    @objc func audioSessionInterruped(notification: NSNotification) {
        guard let interruptionDict = notification.userInfo,
              let interruptionType = interruptionDict[AVAudioSessionInterruptionTypeKey] as? UInt
        else {
            return
        }
    ...
    @objc func applicationDidChangeActive(notification: NSNotification) {
        appIsActive = notification.name.rawValue == UIApplication.didBecomeActiveNotification.rawValue
